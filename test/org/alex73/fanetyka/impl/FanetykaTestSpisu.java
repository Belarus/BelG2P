...
public class FanetykaTestSpisu {
    ...
    public void test() throws Exception {
        ...
        Set<String> f0 = new HashSet<>(Files.readAllLines(Path.of("test/org/alex73/fanetyka/impl/spraudzanaSkip.txt")));
        ...
        for (int i = 0; i < in.size(); i++) {
            ...
            String word = in.get(i).substring(0, p).toLowerCase().trim().replaceFirst("^\\[?[0-9]+[абab]?\\.\\]?", "").trim().replace("%", "´")
                    .replace("ё", "ё´").replace("´´", "´").replace("!", "").replace("?", "").replace("@", "");
            if (f0.contains(word)) {
                continue;
            }
            ...
            try {
                ...
                if (!expected.equals(tr)) {
                    ...
                    count++;
                    if (expected.equals(tr.replace("´", ""))) {
                        in.set(i, in.get(i).replace(expected, tr));
                    }
                }
            } catch (Exception ex) {
